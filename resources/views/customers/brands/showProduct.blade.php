                ...
                <form action="{{ url()->current() }}">
                    <div id="filter-by-price">
                        ...
                        <div class="slider-labels">
                            <strong>{{ __('m_price') }}: </strong> <span id="slider-range-value1"></span>đ - <span id="slider-range-value2"></span>đ

                        </div>
                        <div class="row">
                            <div class="col-sm-12">
                                <input id="min-value" type="hidden" name="min_value" value="{{ request()->min_value ?? $min_price }}">
                                <input id="max-value" type="hidden" name="max_value" value="{{ request()->max_value ?? $max_price }}">
                            </div>
                        </div>
                    </div>
        ...
        <div class="col-9">
            <div class="row">
                ...
                @else
                    <div class="col text-center">
                        <img height="200" src="{{ asset('images/logo/no-product.png') }}" alt="">
                        <h2>{{ __('no product in', ['attr' => strtolower(__('brand'))]) }}</h2>
                    </div>
                @endif
            </div>
        </div>
...
@section('script')
    <script>
        $(document).ready(function() {
            let min_price = Number($('#min-max').attr('data-min'))
            let max_price = Number($('#min-max').attr('data-max'))
            let min_value = Number($('#min-value').val())
            let max_value = Number($('#max-value').val())
            ...
            rangeSlider.noUiSlider.on('update', function(values, handle) {
                min = number_format(values[0], '.', ',')
                max = number_format(values[1], '.', ',')
                $('#slider-range-value1').html(min);
                $('#slider-range-value2').html(max);
